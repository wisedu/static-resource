...
WIS_EMAP_SERV.getModel = function(pagePath, actionID, type) {
    ...
    if(type == "search"){
        pageMeta.controls.map(function(item) {
        ...
        WIS_EMAP_SERV.modelName = pageMeta.modelName;
        WIS_EMAP_SERV.appName = pageMeta.appName;
        WIS_EMAP_SERV.url = pageMeta.url;
        WIS_EMAP_SERV.name = pageMeta.name;
...
function getTotalRecordsMock(data, action){
    return data.datas[action].totalSize || data.datas[action].total_size;
}