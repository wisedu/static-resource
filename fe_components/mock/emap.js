...
(function($) {
	...
	function render(settings, $element, pageNumber, pageSize) {
		...
		var currentModel = getCurrentModel(settings.pagePath, settings.action);
		...
		$.ajax({
			...
			var pagenum = 1;
			$element.find('.bh-emapCard-card-list').empty();
			for (var i = 0; i < rows.length; i++) {
				$element.find('.bh-emapCard-card-list').append(compile(settings.template, rows[i]));
			}
	...
	function getCurrentModel(modelPath, action) {
		var model = null;
	...
	function compile(template, data) {
		...
		if (typeof(template) === 'object') {
			...
			compileTemplate = Hogan.compile(settings.template);
		}
...
(function () {   
    ...
    function _create(instance){
        ...
        var custom_rendered = jqxOptions.rendered;
    ...
    function _genColums(instance, jqxOptions, custom_ready, custom_rendered){
        ...
        instance.$element.attr("emap-pagePath", instance.settings.pagePath)
        .attr("emap-action", instance.settings.action)
        .attr("emap-url", WIS_EMAP_SERV.url)
        .attr("emap-name", WIS_EMAP_SERV.name)
        .attr("emap-app-name", WIS_EMAP_SERV.appName)
        .attr("emap-model-name", WIS_EMAP_SERV.modelName);
        ...
        for(var i = 0; i < cusColLen; i++){
            ...
            if(colIndex == 'last'){
                ...
                if(type != 'link'){
                    ...
                    linkCol.push({colIndex: colIndex, column: customColumns[i]});
                }
                
            }
        }
        //删除datamodel中隐藏字段,并保存至source的datafields数组中
        for(var m = 0; m < datamodel.length; m++){
            if((typeof datamodel[m].get == 'function' && datamodel[m].get('hidden') === true) || datamodel[m].hidden === true || datamodel[m]['grid.hidden'] === true){
                instance.source.datafields.push({name: datamodel[m].name, type: 'string'});
                if(typeof datamodel[m].url != 'undefined'){
                    instance.source.datafields.push({name: datamodel[m].name + '_DISPLAY', type: 'string'});
                }
                datamodel.splice(m, 1);
                m--;
            }
        }
        ...
        if(lastcolumn != null){
            newmodel.push({custom: lastcolumn});
        }
        ...
        for(var n = 0; n < newmodel.length; n++){
            ...
            if(typeof newmodel[n].name != 'undefined'){
                instance.source.datafields.push({name: newmodel[n].name, type: 'string'});
            }
            ...
            if(newmodel[n].custom == undefined){
                ...
                columns.push(_genCustomColumns(type, instance, jqxOptions, showCheckAll, widthObj, newmodel[n], datafield, custom_ready, custom_rendered));
            }
    ...
    function _genCustomColumns(type, instance, jqxOptions, showCheckAll, widthObj, model, datafield, custom_ready, custom_rendered){
        ...
        case 'checkbox': 
            column = {
                text: 'checkbox', width: '32px', cellsAlign: 'center', align: 'center',
         ...
         if (setting.url) {
             ...
             var dataAdapter = new $.jqx.dataAdapter(source, {
                 beforeLoadComplete: function(loaded_data, original_data) {
                     var new_data = [{ name: "请选择...", pId: "...", value: "" }];
                     for (var i = 0; i < loaded_data.length; i++) {
                         var item = loaded_data[i];
                         new_data.push({ id: item.id, name: item.name, pId: "...", value: item.id });
                         if (item.isParent === 1) {
                             var sub_item = { id: item.id + '_load', name: "加载中...", pId: item.id, value: item.id };
                                 ...
                                 success: function(data, status, xhr) {
                                     ...
                                     if (items.datas && items.datas.code && items.datas.code.rows) {
                                         ...
                                         for (var i = nodes.length - 1; i >= 0; i--) {
                                            ...
                                            var treenode = { label: itemLabel, value: nodes[i].id };
        ...
        EmapFileUpload.prototype.saveTempFile = function () {
            ...
            $('.bh-file-upload-name-a', this.options.loadedCon).each(function(){
                ...
                if (new RegExp('getTempFile').test(href)) {
                    var fileId = $(this).closest('.bh-file-upload-file').data('fileid');
...
(function () {
    var Plugin,
        _render, _renderFormWrap,
        _renderReadonlyFormStructure, _renderEditFormStructure, _sortModel, _getAttr, _emapFormDo;  //插件的私有方法
    ...
    Plugin = (function () {
        ...
        function Plugin(element, options) {
            ...
            this.$element.attr("emap-role", "datatable");
    ...
    _renderReadonlyFormStructure = function (form, data, cols) {
        ...
        $(data).each(function () {
            ...
            if (attr.xtype == "textarea") attr.col = cols;
    ...
    _renderEditFormStructure = function (form, data, model) {
        $(data).each(function () {
            ...
            switch (attr.xtype) {
                ...
                case "text" :
                    attr.xtype = "text";
                    controlHtml = '<input class="bh-form-control" data-name="{{name}}" name="{{name}}" xtype="{{xtype}}" type="{{xtype}}" {{checkType}}  {{dataSize}} {{checkSize}} {{checkExp}} ' + (attr.inputReadonly ? 'readOnly' : '') + '  />';
                    break;
                case "textarea" :
                    controlHtml = '<textarea xtype="{{xtype}}" class="bh-form-control" rows="3" data-name="{{name}}" {{checkType}} {{dataSize}} {{checkSize}} {{checkExp}} ' + (attr.inputReadonly ? 'readOnly' : '') + ' ></textarea>';
                    break;
                case "selecttable" :
                case "select" :
                case "tree" :
                ...
                case "multi-select" :
                case "div":
                    controlHtml = '<div xtype="{{xtype}}" data-name="{{name}}" {{url}} {{format}} {{checkType}} {{JSONParam}} {{checkSize}} data-disabled={{inputReadonly}}></div>';
                    break;
                case "radiolist" :
                    controlHtml = '<div xtype="{{xtype}}" class="bh-radio jqx-radio-group" data-name="{{name}}" {{url}} {{checkSize}} data-disabled={{inputReadonly}}></div>';
                    break;
                case "checkboxlist" :
                    controlHtml = '<div xtype="{{xtype}}" class="bh-checkbox" data-name="{{name}}" {{checkType}} {{url}} {{checkSize}} data-disabled={{inputReadonly}}></div>';
                    break;
            }
...
(function($) {
	...
	function switchGrid(type, settings, $element) {
		currentView = type;
		if (type == 'list') {
	 ...
	 * 定义一个插件 plugin
	 */
	$.fn.emapGrid = function(options, params) {
		var instance;
