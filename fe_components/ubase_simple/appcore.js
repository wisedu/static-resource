...
define('utils', [
    ...
    var req = require;
    var utils = {
        loadCompiledPage: function (relativePath, commomModuleName) {
            ...
            var url = null;
            if (commomModuleName) {
                url = './modules/common/' + commomModuleName + '/' + relativePath + '.html';
            } else {
        ...
        window: function (options) {
            ...
            var title = options.title, content = options.content, btns = options.buttons, callback = options.callback;
            var $dom = $('<div style="padding-bottom:72px;position:fixed;">' + '<div class="head"></div>' + '<div>' + '<div class="content"></div>' + '<div id="buttons" style="position: absolute;bottom:32px;width: 100%;left: 0;float: right;padding: 0 24px;">' + '<hr style="border:none;border-top: 1px solid #efefef;margin-bottom: 10px;">' + '</div>' + '</div>' + '</div>');
            $('body').append($dom);
            $('#buttons', $dom).hide();
            ...
            $dom.jqxWindow($.extend({
                ...
                isModal: true,
                modalOpacity: 0.3,
                height: options.height || params.height || '100%',
                width: options.width || params.width || 500,
                autoOpen: false
...
define('resourceConfig', [
    ...
    var config = {
        'PUBLIC_CSS': [
            ...
            '/fe_components/jqwidget/{{theme}}/bh.min.css',
            '/fe_components/jqwidget/{{theme}}/bh-scenes.min.css'
        ],
        'PUBLIC_BASE_JS': [],
        'PUBLIC_NORMAL_JS': [
            '/fe_components/bh.min.js',
            '/fe_components/we/we.pagination.js'
        ],
        'DEBUG_JS': [],
        ...
        loadPublicCss: function () {
            ...
            for (var i = 0; i < publicCss.length; i++) {
                if (regEx.test(publicCss[i])) {
                    loadCss(cdn + publicCss[i].replace(/\{\{theme\}\}/, theme));
                } else {
                    loadCss(publicCss[i]);
                }
            }
