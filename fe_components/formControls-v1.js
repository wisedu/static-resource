...
var formControls = {
	init : function($form, contextPath){
		...
		$("input[xtype=dateEditorItem]",$form).each(function(){
			if($(this).attr("format") == "yyyy-MM"){
		...
		$("input[xtype=comboBoxItem]",$form).each(function(){
			var $org_input = $(this);
			
			var _url = $org_input.data("url");
			
			if(_url.indexOf("?tree=") == -1)
				return;
					...
					"core" : {
						...
						"data" : function(node, cb){
							...
							$.post(ajaxurl,params,function(resp){
								resp = eval("(" + resp + ")");
								if(resp.code == "0"){
									var rows = resp.result.datas.code.rows;
									if(rows && rows.length > 0){
										...
										for(var i=0;i<rows.length;i++){
											...
											var _node = {
												...
												icon : rows[i]["isParent"] == 1 ? "" : "jstree-file"
											};
											arr.push(_node);
										}
										var temps = arr;
...
//					var opts = inst.options || {};
					...
					if(window.checkRootName && $input.data("url").indexOf(window.checkRootName) > -1){
						
					}else{
						if((_onlyLeaf && !window.checkRoot) && !data.instance.is_leaf(value)){
							return false;
						}
					}
						...
						if(url){
							$.ajax({
								...
								success: function(res) {
									var datas = undefined;
									if(res.result && res.result.datas && res.result.datas.code && res.result.datas.code.rows)
										datas = res.result.datas.code.rows;
									if(datas)
										callback(datas);										
								}
							});
