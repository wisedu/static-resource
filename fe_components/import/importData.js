	...
	$(this).fileupload({
		...
		add: function (e, data) {
			...
			if (e.target.files) {
				...
				step1Contents.eq(1).find("span.upload-file-size").text("(" + file[0].size/1024 + "k)");
			}
									...
									success : function (json) {
										...
										if (json.status == 1) {
											$(".upload-loading-bar div").stop().animate({"width": "100%"}, 500, function(){
												...
												$("div.upload-step:eq(2)").find(".upload-export").attr("href", contextPath + "/sys/emapcomponent/file/getAttachmentFile/" + json.attachment + ".do");
